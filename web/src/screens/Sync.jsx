        ...
        <div className="mt-3 space-y-2 flex-1">
          {cola.length === 0 && (
            <div className="border border-borde bg-t3 px-4 py-6 text-center">
              <p className="text-base font-bold text-tinta">La cola está vacía</p>
          ...
          {cola.map((i, k) => (
            <div key={i.uuid_offline}
              ...
              <p className="text-sm text-apagado font-mono mt-0.5">
                uuid  {i.uuid_offline.slice(0, 4)}…{i.uuid_offline.slice(-3)}
              </p>
              <p className="text-sm text-tinta mt-0.5">{i.detalle || TEXTO[i.estado]}</p>
            </div>
          ))}
        </div>
